...
ADC 평균화: read_adc() 함수 내부에서 센서값을 4회 측정하여 평균값을 사용, 전기적 노이즈 감쇠.
...
해결: 하드웨어를 다시 납땜하는 대신, 소프트웨어 드라이버에서 오른쪽 모터 인덱스(PR) 제어 방향을 역으로 매핑하여 해결.

Code: if (dR > 0) PR = (PR - 1) & 7; // 전진 명령 시 인덱스 감소
